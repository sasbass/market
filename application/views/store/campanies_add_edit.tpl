...
<div class="grid_9">
	...
	{include file="common/message.tpl"}
		...
		<label for="ident_num">Идент. номер по ДДС <strong><em>*</em></strong></label><br/>
		<input id="ident_num" name="ident_num" value="{$edit->ident_num}" type="text" class="in"/><br/>
		
		<label for="city_id">Място на регистрация <strong><em>*</em></strong></label><br/>
		...
		<label for="phone">Телефон за връзка <strong><em>*</em></strong></label><br/>
		<input id="phone" name="phone" value="{$edit->phone}" type="text" class="in"/><br/>
		
		<label for="email">Email <strong><em>*</em></strong></label><br/>
		<input id="email" name="email" value="{$edit->email}" type="text" class="in"/><br/>
		
		<input name="add" value="{if $action == 'add'}ДОБАВИ ФИРМА{else}ЗАПАЗИ ПРОМЕНИТЕ{/if}" type="submit" class="bt"/>
	</fieldset>
</form>
</div>
{/block}